...
| **品类拖拽排序** | ✅ | PC 端和移动端均支持拖拽排序，仅允许同一父级内部排序 |
| **谷子排序功能** | ✅ | 支持前移/后移排序，支持跨页排序，智能边界检测 |
| **右键快捷操作** | ✅ | 云展柜页面支持右键菜单编辑/删除/排序 |
| **多展柜管理** | ✅ | 支持创建多个独立展柜，内容编排与自定义封面 |
| **主题管理功能** | ✅ | 独立页面管理集合主题，支持主题展示图库 |
| **身份认证系统** | ✅ | 登录流程、Token 存储与请求自动注入已完成 |
| **刷新功能** | ✅ | 悬浮刷新按钮，一键刷新当前搜索结果 |
| **移动端适配** | ✅ | 响应式导航栏、折叠菜单、卡片式布局、底部导航栏 |
| **移动端手势优化** | ✅ | 详情抽屉拖拽手势、半屏/全屏切换 |
| **路由标题管理** | ✅ | 路由切换时自动更新页面标题 |
| **错误处理** | ✅ | 429 限流、409 冲突、404 未找到等友好提示 |
| **数据统计看板** | ✅ | 基于 `/api/goods/stats/` 的多维统计与可视化（状态、官谷/同人、作品类型、IP TopN、品类 TopN 等） |

### 🔄 待完善功能

#### 1. 交互与体验 (UX & Interaction)
- **批量操作能力**：支持批量移动位置、修改状态、批量删除以及批量编辑标签。
- **全局搜索增强**：支持 `Ctrl+K` 快速唤起全局搜索，支持 IP/角色/位置的快速跳转。
- **离线访问支持 (PWA)**：集成 PWA，支持在弱网或离线环境下查看已缓存的资产数据。
- **自定义快捷键**：为常用操作（如新增谷子、保存详情、切换标签页）提供键盘快捷键支持。

#### 2. 功能增强 (Feature Enhancements)
- **数据导出与备份**：支持将资产数据导出为 Excel 或 CSV 格式，方便用户进行离线整理。
- **资产变动日志**：记录资产的每一次位置迁移、状态变更及关键信息修改，提供历史追溯能力。
- **公开分享功能**：支持生成特定展柜或集合的公开预览链接，支持只读模式下的社交分享。

#### 3. 性能优化 (Performance)
- **虚拟列表渲染**：针对超大规模资产列表，引入虚拟滚动（Virtual List）技术，提升滚动流畅度。
- **图片加载策略优化**：实现图片的渐进式加载与预加载，并根据设备屏幕密度智能请求缩略图。
