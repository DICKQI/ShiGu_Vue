...
- **多展柜管理**：支持创建多个独立展柜，用于展示不同主题或系列的收藏
- **内容编排**：将谷仓中的谷子添加到指定展柜，并支持自定义排序（拖拽或右键菜单）
- **自定义封面**：支持上传自定义图片作为展柜封面，美化展示效果
- **马赛克预览**：首页展柜卡片自动生成前 4 张藏品的马赛克缩略图预览
- **分类管理**：展柜内支持二级分类，方便对展柜内的大量藏品进行进一步组织
...
独立页面管理谷子品类，支持层级化管理与拖拽排序。

| 功能 | 说明 | 技术实现 |
|------|------|----------|
| � **CRUD 操作** | 品类的增删改查，支持父级品类选择 | RESTful API + 表单校验 |
| 🔄 **拖拽排序** | 支持同一层级下的品类拖拽排序，实时同步到后端 | `sortablejs` + 批量更新接口 |
| 🎨 **颜色标签** | 支持为品类设置颜色标签，在列表中增强视觉识别 | CSS 变量 + 动态样式 |
...
用于管理藏品的集合主题（如：夏日主题、节日主题、XX 周年等），提供除品类外的另一维度分类能力。

| 功能 | 说明 | 技术实现 |
|------|------|----------|
| 📝 **CRUD 操作** | 主题的增删改查 | RESTful API + 表单校验 |
| 🖼️ **主题图库** | 支持为每个主题上传多张展示图片，并设置图片标签 | 图片上传 API + 标签编辑 |
| 🔍 **搜索定位** | 支持按主题名称或描述快速搜索 | 前端过滤 |
| � **响应式视图** | PC 端表格展示，移动端卡片展示，支持下拉刷新 | 响应式组件 + 触摸事件处理 |

**组件依赖**：`ThemeManagement.vue`
